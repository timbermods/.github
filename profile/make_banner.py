...
wherever GitHub shows it. Trees, logs and sticks are placed with a fixed seed, so a re-run gives the same banner.
    ...
    python profile/make_banner.py        (needs: pip install fonttools brotli)
"""
...
W, H = 1280, 360
rnd = random.Random(1280360)
f1 = lambda v: f"{v:.1f}".rstrip("0").rstrip(".")


def text_path(font_file, s, x, y, size, tracking=0.0):
    """The string as one SVG path in the given font, left-aligned at x on baseline y; returns (d, width)."""
    font = TTFont(os.path.join(HERE, "src", font_file))
    gs, cmap, upem = font.getGlyphSet(), font.getBestCmap(), font["head"].unitsPerEm
    scale = size / upem
    pen = SVGPathPen(gs, ntos=f1)
    cursor = 0.0
    for ch in s:
        g = gs[cmap[ord(ch)]]
        g.draw(TransformPen(pen, (scale, 0, 0, -scale, x + cursor, y)))
        cursor += g.width * scale + tracking
    return pen.getCommands(), cursor - tracking


def pine(x, base, h, colour, tiers=3):
    """A stylised pine: stacked triangles on a short trunk."""
    w = h * .42
...
def ridge_y(x):
    ...
    if x < 470:  # the hills fall away under the title, so it stays on open sky
        y0 = 214 - 16 * math.sin(470 / 150) - 9 * math.sin(470 / 61 + 1.3)
        y = y0 + (470 - x) * .45
    return y


# ---------------------------------------------------------------- the dam: a heap of cut log ends
...
def dam():
    ...
    # water seeping through the gaps, down the downstream face
    seep = "".join(f'<path d="M{f1(x)} {f1(y)} q-4 {f1((306 - y) / 2)} -{f1(8 + i * 3)} {f1(306 - y)}" fill="none" stroke="#a9cfc0" stroke-width="1.6" stroke-linecap="round" opacity=".55"/>'
                   for i, (x, y) in enumerate([(648, 268), (634, 282), (662, 262), (620, 292)]))
    foam = "".join(f'<ellipse cx="{f1(x)}" cy="306" rx="{f1(rx)}" ry="1.8" fill="#d8e8dd" opacity=".45"/>' for x, rx in [(596, 14), (612, 8), (628, 10)])
    return (f'<clipPath id="damclip"><polygon points="{pts}"/></clipPath>'
            f'<polygon points="{pts}" fill="#2f2117"/>'
            f'<g clip-path="url(#damclip)">{logs}</g>{sticks}{seep}{foam}')


# ---------------------------------------------------------------- the lodge: a dome of sticks in the pond
def lodge():
...
# ---------------------------------------------------------------- a beaver swimming home with a leafy branch
BEAVER = """
<g>
...
WAKE = "".join(f'<path d="M{x} {y} q{dx / 2} {q} {dx} 0" fill="none" stroke="#e6dbb8" stroke-width="1.3" opacity="{o}" stroke-linecap="round"/>'
...
# ---------------------------------------------------------------- birches and a gnawed stump on the right bank
def birch(x, base, h, lean):
    ...
    crown = "".join(f'<ellipse cx="{f1(x + lean + dx)}" cy="{f1(base - h + dy)}" rx="{rx}" ry="{ry}" fill="{c}"/>'
                    for dx, dy, rx, ry, c in [(-16, 8, 20, 16, "#4f7a3a"), (14, 4, 22, 17, "#5f8a44"), (-2, -12, 22, 18, "#6f9a4a"),
                                              (4, 20, 18, 14, "#557f3e"), (-8, -2, 14, 11, "#8fb35e"), (12, -6, 10, 8, "#a3c46a")])
    return trunk + marks + crown


STUMP = """
<path d="M1120 360 L1128 326 C1160 314 1220 308 1280 306 L1280 360 Z" fill="#18271c"/>
...
def build():
    ...
    svg = f'''<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {W} {H}" width="{W}" height="{H}" role="img" aria-label="Timbermods: mods for Timberborn. A river valley at dusk: a beaver swims home with a leafy branch past its stick lodge, above a dam of logs, among pines and birches.">
  <defs>
    <clipPath id="frame"><rect width="{W}" height="{H}" rx="16"/></clipPath>
    <linearGradient id="sky" x1="0" y1="0" x2="0" y2="1">
      <stop offset="0" stop-color="#0d1a14"/><stop offset=".5" stop-color="#16281e"/><stop offset=".78" stop-color="#2a3a26"/><stop offset="1" stop-color="#3a3f26"/>
    </linearGradient>
    <radialGradient id="dusk" cx="980" cy="240" r="470" gradientUnits="userSpaceOnUse">
      <stop offset="0" stop-color="#eaa94f" stop-opacity=".62"/><stop offset=".4" stop-color="#b8743a" stop-opacity=".22"/><stop offset="1" stop-color="#b8743a" stop-opacity="0"/>
    </radialGradient>
    <linearGradient id="pond" x1="0" y1="262" x2="0" y2="360" gradientUnits="userSpaceOnUse">
      <stop offset="0" stop-color="#7c6c3c"/><stop offset=".22" stop-color="#3f5a3f"/><stop offset="1" stop-color="#1c3029"/>
    </linearGradient>
    <linearGradient id="river" x1="0" y1="304" x2="0" y2="360" gradientUnits="userSpaceOnUse">
      <stop offset="0" stop-color="#445a3e"/><stop offset=".06" stop-color="#364f39"/><stop offset="1" stop-color="#1c3029"/>
    </linearGradient>
    <radialGradient id="mist">
      <stop offset="0" stop-color="#f3e3b8" stop-opacity=".12"/><stop offset="1" stop-color="#f3e3b8" stop-opacity="0"/>
    </radialGradient>
  ...
  <g clip-path="url(#frame)">
    ...
    {stars}
    <path d="M1196 44 a19 19 0 1 0 17 26 a15 15 0 1 1 -17 -26 Z" fill="#f3e3b8" opacity=".85"/>
    <path d="M300 360 L300 {f1(ridge_y(300))} {" ".join(f"L{x} {f1(ridge_y(x))}" for x in range(300, 1290, 10))} L1280 360 Z" fill="#1d3226"/>
    {treeline(330, 1280, ridge_y, 16, 32, "#223829", step=(8, 13))}
    {treeline(770, 1290, lambda x: 263, 60, 128, "#1b3023", step=(12, 22), round_share=.28)}
    <rect x="740" y="262" width="540" height="98" fill="url(#pond)"/>
    <path d="M740 262.5 H1280" stroke="#e6c885" stroke-width="1.2" opacity=".55"/>
    <ellipse cx="1010" cy="258" rx="300" ry="20" fill="url(#mist)"/>
    {lodge()}
    {WAKE}
    {BEAVER}
    {dam()}
    {treeline(-6, 600, lambda x: 305, 26, 58, "#15261b", step=(9, 16), round_share=.2)}
    <rect x="0" y="304" width="790" height="56" fill="url(#river)"/>
    <path d="M0 304.5 H596" stroke="#d6c28a" stroke-width="1" opacity=".35"/>
    {"".join(f'<path d="M{x} {y} h{w}" stroke="#cfdcc0" stroke-width="1" opacity=".22" stroke-linecap="round"/>' for x, y, w in [(470, 314, 60), (380, 320, 44), (540, 322, 36), (1000, 290, 70), (1120, 300, 50), (860, 296, 40), (230, 318, 50)])}
    <path d="M0 360 L0 {f1(left_bank(0))} {" ".join(f"L{x} {f1(left_bank(x))}" for x in range(0, 470, 10))} L470 360 Z" fill="#111e17"/>
    {grass(4, 460, left_bank, "#2c4a2e", 60)}
    {STUMP}
    {grass(1128, 1276, lambda x: 322 - (x - 1128) * .1, "#2f5230", 26)}
    {birch(1222, 314, 196, 4)}
...
if __name__ == "__main__":
    ...
    print("made banner.svg,", len(svg) // 1024, "KB; title ends at x =", round(right))
