...
| `profile/avatar/` | The organization's avatar: the Timbermods mark (a pine on a cut log end, as in the banner and the catalog), on the banner's forest green. `make_avatar.py` draws the SVG; upload `timbermods-avatar-1024.png` at the org's Settings → Profile picture |
